...
public class LockManager
{
    ...
    private static int DEADLOCK_TIMEOUT = 10000*3;
    ...
    public boolean Lock(int xid, String strData, int lockType) throws DeadlockException/*,RedundantLockRequestException*/ {
                ...
                synchronized (this.lockTable) {
                    ...
                    if (!bConflict) {
                        ...
                        if (bConvert.get(0) == true) {
                        	...
                        	//(TrxnObj)this.lockTable.get(trxnObj);
                            DataObj dataObj2;
                            int size = vect1.size();
                            
                            for (int i = 0; i < size; i++) 
                            {
                                dataObj2 = (DataObj) vect1.elementAt(i);
                                if (dataObj.strData.equals(dataObj2.strData) && dataObj.getXId() == dataObj2.getXId())
                                {
                                	dataObj2.setLockType(DataObj.WRITE);
                                	//this.lockTable.remove(dataObj2);
                                	//this.lockTable.add(dataObj);
                                	System.out.println("Lock Upgraded for DataObj.");
                                	break;
                                }
                            }
                            
                            Vector vect2 = this.lockTable.elements(trxnObj);
                        	//(TrxnObj)this.lockTable.get(trxnObj);
                            TrxnObj trxnObj2;
                            size = vect2.size();
                            ...
                            for (int i = 0; i < size; i++) 
                            {
                            	trxnObj2 = (TrxnObj)vect2.elementAt(i);
                                if (trxnObj.strData.equals(trxnObj2.strData) && trxnObj2.getXId() == trxnObj2.getXId())
                                {
                                	trxnObj2.setLockType(TrxnObj.WRITE);
                                	System.out.println("Lock Upgraded for TrxnObj.");
                                	break;
                                }
                            }
    ...
    public boolean  UnlockAll(int xid) {
        ...
        System.out.println("UnlockAll called for Transaction "+xid);
        ...
        synchronized (this.lockTable) {
            ...
            for (int i = (size - 1); i >= 0; i--) {
                ...
                System.out.println("Removing Lock on "+dataObj.strData);
                        ...
                        if (waitObj.getLockType() == LockManager.WRITE) {
                            if (j == 0) {
                                ...
                                if (vect1.size () == 0) {
                                	...
                                	System.out.println(vect1.size()+" trxns wants the lock of "+dataObj.xid);
                                	DataObj new_dataObj = new DataObj(waitObj.getXId(), waitObj.getDataName(), waitObj.getLockType());
                                	TrxnObj new_trxnObj = new TrxnObj(waitObj.getXId(), waitObj.getDataName(), waitObj.getLockType());
                                	Vector new_vect = this.lockTable.elements(new_dataObj);
                                	Vector new_vect1 = this.lockTable.elements(new_trxnObj);
                                	for (int itr = 0; itr < new_vect.size(); itr++) 
                                    {
                                		DataObj new_dataObj2 = (DataObj) new_vect.elementAt(itr);
                                        if (new_dataObj2.getXId() == waitObj.getXId())
                                        {
                                        	new_dataObj2.setLockType(DataObj.WRITE);
                                        	//this.lockTable.remove(dataObj2);
                                        	//this.lockTable.add(dataObj);
                                        	System.out.println("Lock Upgraded for DataObj.");
                                        	break;
                                	...
                                	for (int itr = 0; itr < new_vect1.size(); itr++) 
                                    {
                                		TrxnObj new_trxnObj2 = (TrxnObj) new_vect1.elementAt(itr);
                                        if (new_trxnObj2.getXId() == waitObj.getXId())
                                        {
                                        	new_trxnObj2.setLockType(DataObj.WRITE);
                                        	//this.lockTable.remove(dataObj2);
                                        	//this.lockTable.add(dataObj);
                                        	System.out.println("Lock Upgraded for TrxnObj.");
                                        	break;
                                	...
                                	/*if(vect1.size()==1)
                                	{
                                		...
                                		DataObj dataObj2 = (DataObj) vect1.elementAt(0);
                                        if (dataObj.strData.equals(dataObj2.strData))
                                        {
                                        	dataObj2.setLockType(DataObj.WRITE);
                                        	System.out.println("Lock Upgraded.");
                                        	
                                        }
                                        Vector vect2 = this.lockTable.elements(trxnObj);
                                        if(vect2.size()>0)
                                        {
                                        TrxnObj trxnObj2 = (TrxnObj) vect2.elementAt(0);
                                        if (trxnObj.strData.equals(trxnObj2.strData))
                                        {
                                        	trxnObj2.setLockType(TrxnObj.WRITE);
                                        	System.out.println("Lock Upgraded.");
    ...
    private boolean LockConflict(DataObj dataObj, BitSet bitset) throws DeadlockException, RedundantLockRequestException {
    	//System.out.println("In LockConflict.");
            ...
            if (dataObj.getXId() == dataObj2.getXId()) {
                ...
                if (dataObj.getLockType() == DataObj.READ) {
                    if (dataObj2.getLockType() == DataObj.WRITE) {
                    	...
                    	System.out.println(dataObj.xid+" wants read on "+dataObj.strData+" " +
                    			"but"+dataObj2.xid+" has write on "+dataObj2.strData);
                	...
                	System.out.println(dataObj.xid+" wants write on "+dataObj.strData+" " +
                			"but"+dataObj2.xid+" has read/write on "+dataObj2.strData);
