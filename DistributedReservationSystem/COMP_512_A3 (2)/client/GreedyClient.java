...
public class GreedyClient {

    class Tuple2 {
        int x;
        int y;
        public Tuple2(int m, int n) {
            x = m;
            y = n;
        }
    }
    ...
    class PerformanceMeter {
        ArrayList<Long> responseTimeSeq = new ArrayList<Long>();

        long startStamp = 0;
        long endStamp = 0;

        public void markStartStamp() {
            startStamp = System.currentTimeMillis();
        }

        public void markEndStamp() {
            endStamp = System.currentTimeMillis();
            responseTimeSeq.add(endStamp - startStamp);
        }

        public long getLastTransactionCost() {
            return endStamp - startStamp;
        }

	public double getAverage(int offset) {
		double sum = 0;
		for (int i = offset; i < responseTimeSeq.size(); i++) {
			sum += (double)responseTimeSeq.get(i);	
		}
		return sum / (responseTimeSeq.size() - offset);
    ...
    public PerformanceMeter pm;

    public GreedyClient(String ip, int portnum) {
        server = ip;
        port = portnum;
        pm = new PerformanceMeter();
    }

    public boolean connectToRemoteObj() {
        try
        {
            // get a reference to the rmiregistry
            Registry registry = LocateRegistry.getRegistry(server, port);
            // get the proxy and the remote reference by rmiregistry lookup
            rm = (ResourceManager) registry.lookup("Group28ResourceManager");
    ...
    private int sendCommand(String command) {
		...
		try{
			int trxnID=getInt(arguments.elementAt(1));
     ...
     * @param dataset
     * @param iterationum
     * @param transactionPerSecond
     * @param maxcustomerid
     * @param maxflightnumber
     * @param locationlist
     */
    public void testPerformance(String loadconfpath, String dataset, int iterationum, float transactionPerSecond,
                                int maxcustomerid, int maxflightnumber, ArrayList<String> locationlist) {
        //HashMap<Integer, Integer> loadconf = new HashMap<Integer, Integer>();
        ArrayList<Tuple2> loadconf = new ArrayList<Tuple2>();
        //build the load map
        Random r = new Random();
        //configure load types
        try {
            FileInputStream fstream = new FileInputStream(loadconfpath);
...
/*            while ((strLine = br.readLine()) != null)   {
            ...
            int loadidx = 0;
            int nextTime;
            int base = (int)(1000 / transactionPerSecond - 10.0);
            for (int i = 0; i < iterationum; i++) {
                nextTime = base + r.nextInt(20);
                int weight = loadconf.get(loadidx).y;
                int type = loadconf.get(loadidx).x;
                pm.markStartStamp();
                switch (type) {
                    case 0://reserve flight
                        int flightnum = r.nextInt(maxflightnumber);
                        int customerid = r.nextInt(maxcustomerid);
			String loc = locationlist.get(r.nextInt(locationlist.size()));
			int tid = sendCommand("start");
                        sendCommand("queryflight," + tid + "," + flightnum);
                        sendCommand("reserveflight," + tid + "," + customerid + "," + flightnum);
                        sendCommand("querycar," + tid + "," + loc);
                        sendCommand("reservecar," + tid + "," + customerid + "," + loc);
                        sendCommand("queryroom," + tid + "," + loc);
                        sendCommand("reserveroom," + tid + "," + customerid + "," + loc);
			sendCommand("commit," + tid);
                        break;
          /*          case 1://reserve room
                        String roomloc = locationlist[r.nextInt(locationlist.length - 1)];
                        customerid = r.nextInt(maxcustomerid);
                        sendCommand("queryroom," + i + "," + roomloc);
                        sendCommand("reserveroom," + i + "," + customerid + "," + roomloc);
                        break;
                    case 2://reserve car
                        String carloc = locationlist[r.nextInt(locationlist.length - 1)];
                        customerid = r.nextInt(maxcustomerid);
                        sendCommand("querycar," + i + "," + carloc);
                        sendCommand("reservecar," + i + "," + customerid + "," + carloc);
                        break;
                    case 3://itinerary
                        flightnum = r.nextInt(maxflightnumber);
                        String loc = locationlist[r.nextInt(locationlist.length - 1)];
                        customerid = r.nextInt(maxcustomerid);
                        sendCommand("itinerary," + i + "," + customerid + "," + flightnum + "," + loc + ",true,true");
                        break;*/
                    default:
                        break;
                }
                pm.markEndStamp();
                //sleep
		if (nextTime - pm.getLastTransactionCost() > 0)
                	Thread.sleep(nextTime - pm.getLastTransactionCost());
                if (--weight == 0) {
                    if (loadidx == loadconf.size() - 1) loadidx = 0;
                    else {
                        loadidx++;
                    }
                }
    ...
    public static void main(String args[]) {
        if (args.length < 9) {
            System.out.println("public void testPerformance(String loadconfpath, " +
                    "String dataset, int iterationum, int transactionPerSecond, int maxcustomerid, int maxflightnumber, " +
                    "String[] locationlist");
            System.exit(-1);
        }
        GreedyClient client = new GreedyClient(args[0], Integer.parseInt(args[1]));
        if (!client.connectToRemoteObj()) {
            System.out.println("cannot connect to remote object");
        }
	ArrayList<String> locationlist = new ArrayList<String>();
	try {
	FileInputStream fstream = new FileInputStream(args[8]);
	DataInputStream in = new DataInputStream(fstream);
	BufferedReader br = new BufferedReader(new InputStreamReader(in));
	String strLine;
	while ((strLine = br.readLine()) != null)   {
		locationlist.add(strLine);	
	}
		fstream.close();
		in.close();
		br.close();
	...
	client.testPerformance(args[2], args[3], Integer.parseInt(args[4]), Float.parseFloat(args[5]),
			Integer.parseInt(args[6]), Integer.parseInt(args[7]), locationlist);
	/*public void testPerformance(String loadconfpath, String dataset, int iterationum, int transactionPerSecond,
	  int maxcustomerid, int maxflightnumber, String[] locationlist) {*/
	try{
		// Create file 
		FileWriter ofstream = new FileWriter("average_response_time_multi_rm_multi_client", true);
		BufferedWriter out = new BufferedWriter(ofstream);
		out.write(args[5] + "\t" + (client.pm.getAverage((int)(0.1 * Integer.parseInt(args[4]))) + "\n"));
